...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        sigmas = torch.tensor(
            [.25, .25, .26, .35, .35, .79, .79, .72, .72, .62, .62, 1.07, 1.07, .87, .87, .89, .89, .28, .32, .9],
            device=device) / 10.0
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if self.kpt_label:
                gain[2:46] = torch.tensor(p[i].shape)[22 * [3, 2]]  # xyxy gain
            else:
            ...
            indices.append((b, a, gj.clamp_(0, gain[3].long() - 1),
                            gi.clamp_(0, gain[2].long() - 1)))  # image, anchor, grid indices
